<script>
	...
	import { displayMessage, fillArray, formatDate, getJwtToken } from '../../../../utils';
	import { getProductCategories, updateProductCategory } from '../../../../api-requests/request';
	...
	const deleteProductCategory = (/** @type {string} */ productCategoryId) => {
		const token = getJwtToken();
		try {
			(async () => {
				const deleteResult = await updateProductCategory(
					{ productCategoryId, status: false },
	...
	<div class="w-full flex justify-center">
		<section class="w-11/12">
			...
			<div class="grid grid-cols-12 gap-4">
				<div class="col-span-12 sm:col-span-12 bg-black-gold p-4">
					...
					<tbody>
						{#each productCategories as category, index}
							<tr id="tr">
								<td>{index + 1}</td>
								<td>{category.name}</td>
								<td>{formatDate(category.dateCreated, 'DATE')}</td>
								<td>{category.status ? 'Active' : 'Inactive'}</td>
								<td>
									<div class="flex items-center gap-2">
										<a href="/auth/admin/product-category/{category.id}">
											<svg
										...
										<span
											on:click={() => deleteProductCategory(category.id)}
